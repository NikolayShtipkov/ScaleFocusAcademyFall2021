...
namespace ToDoListApp.View
{
    public class ToDoListMenu
    {
        ...
        public void PromptEditToDoList()
        {
            ...
            var list = _toDoListService.GetListById(idNum);

            bool isShared = _toDoListService.CheckShare(_userService.CurrentUser.Id, list.Id);
            bool isCreator = list.CreatorId == _userService.CurrentUser.Id;

            if (!isNum || (!isCreator && !isShared))
            {
